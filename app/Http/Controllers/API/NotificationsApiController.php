...
class NotificationsApiController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if(isset($type_ids) && count($type_ids)>0){
            $userTokens = UserToken::where('type','customer')->whereIn('user_id',$type_ids)->get()->pluck('fcm_token')->toArray();
        }else{
            $userTokens = UserToken::where('type','customer')->get()->pluck('fcm_token')->toArray();
        }

...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  data: function data() {
    ...
    return _ref = {
      ...
      error: null
    }, _defineProperty(_ref, "parent_id", 0), _defineProperty(_ref, "editedCategoryId", null), _defineProperty(_ref, "selectedCategories", []), _ref;
  },
  watch: {
  ...
  computed: {
    ...
    filteredCategories: function filteredCategories() {
      var _this = this;
      var childIds = this.childrenIds();

      // Check if this.record is defined and has an 'id' property
      if (this.record && this.record.id !== undefined) {
        // Filter out the category being edited from the categories array
        return this.categories.filter(function (category) {
          return category.id !== _this.record.id && category.parent_id !== _this.record.id && (!childIds || !childIds.includes(category.parent_id));
        });
      } else {
        // Handle the case where this.record is null or undefined
  ...
  methods: {
    childrenIds: function childrenIds() {
      var _this2 = this;
      // Check if this.record is defined and has an 'id' property
      if (this.record && this.record.id !== undefined) {
        // Filter categories where parent_id is equal to the id of the record being edited
        var filteredCategories = this.categories.filter(function (category) {
          return category.parent_id === _this2.record.id;
        });

        // Extract the id values from the filtered categories
        var childIds = filteredCategories.map(function (category) {
          return category.id;
        });
        return childIds;
      } else {
